...
(function () {
  ...
  async function saveLog({ actionType, itemName, quantity, unit, totalPrice, workerName }) {
    ...
    if (actionType !== 'in' && actionType !== 'out') return { ok: false, error: 'bad_type' };
  ...
  async function createTestLog() {
    ...
    toast(res.ok ? 'Тестовая запись добавлена' : 'Не удалось: ' + res.error, !res.ok);
    return res;
  }
  ...
  function listenToLogsRealtime(filterType) {
    ...
    local.bizId           = bizId;
    local.logs            = [];
    local.firstPageLastDoc = null;
    local.tailEndDoc       = null;
    local.hasMore          = true;
  ...
  async function loadMoreLogs() {
    ...
    if (local.indexError) return;

    const cursor = local.tailEndDoc || local.firstPageLastDoc;
    if (!cursor) return;

    local.loadingMore = true;
    renderLoadMore(true);

    const { db, collection, query, where, orderBy, limit, startAfter, getDocs } = window.FB;
    const bizId = local.bizId || getBizId();
    const base  = collection(db, 'businesses', bizId, COLLECTION);

    const constraints = [];
    if (local.filter === 'in' || local.filter === 'out') {
      constraints.push(where('actionType', '==', local.filter));
    }
    constraints.push(orderBy('timestamp', 'desc'));
    ...
    try {
      ...
      if (code.includes('failed-precondition') || /index/i.test(msg)) {
        ...
        toast('Не удалось загрузить старые записи', true);
      }
    } finally {
  ...
  function setLiveStatus(status, count) {
    ...
    if (!el) return;
    el.classList.remove('is-live', 'is-error', 'is-connecting', 'is-warn');
    if (status === 'live') {
  ...
  function renderIndexWarning(message) {
    ...
    const url = extractIndexUrl(message);
    const filterName = local.filter === 'in' ? 'Приходы' : local.filter === 'out' ? 'Списания' : 'Все';

    listEl.innerHTML = `
      <div class="wh-hint wh-hint--warn">
        <div class="wh-hint__icon">🔧</div>
        <div class="wh-hint__title">Нужен составной индекс</div>
        <div class="wh-hint__text">
          Фильтр <b>«${escapeHtml(filterName)}»</b> требует индекс Firestore.<br>
          Один раз создайте его — и фильтр заработает навсегда.
        </div>
        ${url
          ? `<a class="wh-hint__link" href="${url}" target="_blank" rel="noopener">Создать индекс →</a>`
          : `<div class="wh-hint__code">Создайте вручную:<br>
Collection ID: warehouse_logs<br>
Field 1: actionType — Ascending<br>
Field 2: timestamp — Descending</div>`}
        <button class="wh-hint__btn" type="button" onclick="WAREHOUSE_LOG.refresh('all')">
          ← Вернуться к «Все»
        </button>
        <div class="wh-hint__note">Фильтр «Все» работает без индекса.</div>
      </div>`;
  }
  ...
  function renderRulesError() {
    ...
    listEl.innerHTML = `
      <div class="wh-hint wh-hint--error">
        ...
        <div class="wh-hint__text">
          Разрешите чтение и запись коллекции <code>warehouse_logs</code>.
        </div>
      </div>`;
  }
  ...
  function renderLoadMore(loading) {
    ...
    if (local.indexError) { wrap.hidden = true; return; }
    if (!local.hasMore && local.logs.length === 0) { wrap.hidden = true; return; }
    if (!local.hasMore) {
      wrap.hidden = false;
      btn.disabled = true;
      btn.textContent = '📜 Это все записи';
      return;
    }
